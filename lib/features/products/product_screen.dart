...
class _ProductsScreenState extends ConsumerState<ProductsScreen> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      ref.read(productListProvider(tenantId).notifier).loadMore();
    }
  }
  ...
  Future<void> _deleteCategory(CategoryModel category) async {
    ...
    try {
      await ref
          .read(categoryListProvider(tenantId).notifier)
  ...
  Future<void> _deleteBrand(BrandModel brand) async {
    ...
    try {
      await ref.read(brandListProvider(tenantId).notifier).deleteBrand(brand.id);
      if (!mounted) return;
  ...
  Future<void> _deleteSupplier(SupplierModel supplier) async {
    ...
    try {
      await ref
          .read(supplierListProvider(tenantId).notifier)
  ...
  void _showFilterSheet(BuildContext context) {
    final categoryState = ref.read(categoryListProvider(ref.read(tenantIdProvider) ?? ''));
    final categories = categoryState.categories;
    final brandState = ref.read(brandListProvider(ref.read(tenantIdProvider) ?? ''));
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ChoiceChip(
                  label: const Text('All'),
                  selected: _selectedBrandId.isEmpty,
                  onSelected: (_) {
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      onRefresh: () => ref.read(productListProvider(tenantId).notifier).refresh(),
            ...
            child: Row(
              children: [
                _MetricChip(
                  ...
                  accentBg: Theme.of(context).colorScheme.primaryContainer,
                  accentFg: Theme.of(context).colorScheme.onPrimaryContainer,
                ),
                ...
                _MetricChip(
                  ...
                  accentBg: Theme.of(context).colorScheme.tertiaryContainer,
                  accentFg: Theme.of(context).colorScheme.onTertiaryContainer,
                ),
                const SizedBox(width: 8),
                _MetricChip(
                  label: 'Low stock',
                  ...
                  accentBg: Theme.of(context).colorScheme.errorContainer,
                  accentFg: Theme.of(context).colorScheme.onErrorContainer,
                ),
                const SizedBox(width: 8),
                _MetricChip(
                  label: 'Featured',
                  ...
                  accentBg: Theme.of(context).colorScheme.secondaryContainer,
                  accentFg: Theme.of(context).colorScheme.onSecondaryContainer,
                ),
              ],
            ),
          ),
          ...
          if (productState.isLoading)
            ...
            Padding(
              ...
              child: Center(
                child: Column(
                  children: [
                    Icon(productState.error!.icon, size: 48, color: productState.error!.color),
                    const SizedBox(height: 16),
                    Text(productState.error!.title, style: Theme.of(context).textTheme.titleMedium),
                    const SizedBox(height: 8),
                    Text(productState.error!.message, textAlign: TextAlign.center),
                    const SizedBox(height: 16),
                    FilledButton(
                      onPressed: () => ref.read(productListProvider(tenantId).notifier).fetchProducts(),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, size: 16, color: accentFg),
          const SizedBox(width: 8),
          Text(
            value,
          ...
          Text(
            label,
            style: Theme.of(context).textTheme.bodySmall?.copyWith(
              color: accentFg.withValues(alpha: 0.75),
